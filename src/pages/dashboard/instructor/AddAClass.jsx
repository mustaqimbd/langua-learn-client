...
const AddAClass = ({ getClass }) => {
  ...
  const initialValues = {
    ...
    availableSeats: getClass?.availableSeats || "",
    ...
    fetch(image_upload_url, { method: "POST", body: formData })
      ...
      .then((result) => {
        if (result.success) {
          ...
          instance.post("/newclass", values).then((data) => {
  ...
  //update class
  const updateClass = (values, { resetForm }) => {
    const image_hosting_token = import.meta.env.VITE_Image_upload_apiKey;
    const image_upload_url = `https://api.imgbb.com/1/upload?key=${image_hosting_token}`;
    const formData = new FormData();
    formData.append("image", image);
    ...
    fetch(image_upload_url, { method: "POST", body: formData })
      ...
      .then((result) => {
        if (result.success) {
          values.image = result.data.display_url;
          instance
            .patch(`/update-class/${getClass._id}`, values)
            .then((data) => {
